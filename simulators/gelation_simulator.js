    ...
    for (const p of particles) {
      ...
      for (const [gx, gy] of allDraws) {
        ...
        const gradient = ctx.createRadialGradient(
          gx - pr * 0.3, gy - pr * 0.3, pr * 0.1,  // inner highlight
          gx, gy, pr * 1.1                          // outer edge
        );
        ...
        const shadowColor = `hsl(0,0%,${Math.max(5, L - 20)}%)`;
  ...
  function drawTransientPlot() {
    ...
    // Vertical grid every 1 time unit (subtle)
    const tStep = 1;
    ...
    // Max cluster size curve with improved styling
    ...
    for (let i = 0; i <= 3; i++) {
      ...
      plotCtx.fillText(t.toFixed(1), tx, areaY + areaH + 8);
    }
    ...
    for (const tv of yTicks) {
      plotCtx.fillText(tv.toFixed(0), areaX - 8, toCanvasY(tv));
    }
    ...
    plotCtx.fillText('Time', areaX + areaW / 2, PH - 4);
  ...
  function drawPhiPlot() {
    ...
    for (const tv of yTicks) {
      phiCtx.fillText(tv.toFixed(0), areaX - 8, toY(tv));
    }
