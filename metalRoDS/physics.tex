...
\subsection{Tangential Force (Mindlin--Coulomb)}

The tangential displacement is accumulated incrementally each timestep:
\begin{equation}
  \bs{\delta}_t^{n+1} = \bs{\delta}_t^n + \bs{v}_t\,\Delta t
\end{equation}

The tangential force before the Coulomb cut-off is
\begin{equation}
  \bs{F}_t' = k_t\,|\bs{\delta}_t|\,\hat{\bs{t}} + \xi_t\,\bs{v}_t
\end{equation}
where $\hat{\bs{t}} = \bs{v}_t/|\bs{v}_t|$.  The Coulomb constraint gives:
\begin{equation}
  \bs{F}_t = \begin{cases}
    \bs{F}_t' & |\bs{F}_t'| \le \mu\,|\bs{F}_n| \\
    \mu\,|\bs{F}_n|\,\hat{\bs{t}} & \text{otherwise}
  \end{cases}
\end{equation}
with $\mu = (\mu_A + \mu_B)/2$.
